...
Lua is not needed for Teeming's build. This package list follows the engine's
Linux setup plus Teeming's font dependencies; a clean-machine install has not
been verified here.
...
even if other checkouts exist beside Teeming. Engine/UI development can opt into
those sibling checkouts with `-DTEEMING_USE_LOCAL_DEPS=ON`; the `TEEMING_GUBSY_DIR`,
`TEEMING_GVIEW_DIR` and `TEEMING_GLAYOUT_DIR` CMake paths then select them.

If an older checkout failed with `Failed to checkout tag` for GView, update Teeming
...
```sh
git clone https://github.com/wegfawefgawefg/teeming.git
cd teeming
./scripts/run.sh
```

If already cloned, just run `./scripts/run.sh`. It configures CMake, builds the
Release `teeming` target incrementally, and launches only if the build succeeds.
...
Set `TEEMING_PRESET=dev` for a debug build. `./scripts/build.sh` builds without launching. Both scripts work from any current directory; `run.sh` passes game arguments through unchanged.

The complete menu flow uses GView and the stone controls from the Splonks C++ menu workspace. Play opens Teeming's GView lobby for player setup, death policy, public room hosting, browsing/joining, and starting a run; Quick Run starts alone. Display, audio, control profiles, button and analog mappings, and input tuning also use GView screens backed by Gubsy's settings and input systems. One player can join from each machine; player storage grows with the connected party. The default direct host port in the lobby is 35355. A CLI host can use a chosen port:
...
The repository and native executable are now `teeming`; build options use `TEEMING_*` and the launcher uses `TEEMING_PRESET`. Older `GAUCHE_PRESET` and cached CMake options are accepted during migration. Native settings and saved loadouts are copied once from the previous application data directory; existing Teeming settings win and the original data remains intact. Browser storage remains `/persistent`.

The room protocol identifier and installed VPS service/account/certificate names retain their existing values for compatibility. Historical notes, user quotations and references to the Rust prototype retain their original names.
