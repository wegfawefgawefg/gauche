...
- [ ] **GEN-CONTROLLER-PLAY:** Main-menu Dev option opens World Gen. Controller bumpers switch Forest floors, a labeled action regenerates, sticks/pad pan and zoom, and fit/annotation controls remain accessible without a mouse. Always display seed; expose copyable seed/revision/settings for feedback. Play enters the exact inspected generation, not a new roll; return/replay retains its original state separately from play mutations. Inspecting must not advance enemies or hazards.
- [ ] **GEN-OPEN-SECTORS:** Allow growth/carving to erase substantial room boundaries and create broad connected sectors. Preserve useful cover, landmarks, terrain contrast and required objectives rather than preserving every planned room silhouette. Include both enclosed and open layouts in ordinary Forest, with stage/theme-dependent frequency and visible pass annotations.
...
- [ ] **GEN-TIMELINE:** Inspector timeline with previous/next pass, scrubber and first/final checkpoints; show pass name, ordinal and selected component/iteration. Retain generation checkpoints (terrain, props, actors/loot, planning overlays and matching annotations), without requiring inverse operations or logging every mutation. Add opt-in finer checkpoints every N iterations or per component for selected carving/growth/placement loops. Highlight added/removed/changed cells and occupants relative to the preceding checkpoint; camera stays fixed while scrubbing. Bound capture memory, expose granularity and distinguish pass progress from elapsed time. Coarse capture is the first deliverable; fine capture may replay deterministically from the seed up to the requested checkpoint, with relevant RNG/planning state retained if resuming mid-generation. Rewinding is inspection only: it must not advance RNG or alter the finished floor. Play uses the completed generation, with clear handling while an intermediate checkpoint is selected. Verify final output matches with capture enabled/disabled and forward/backward views restore the same checkpoint.
- [ ] **GEN-COMPOSE:** Extend GEN-LAYERS with room/landmark alternatives containing smaller weighted component slots, conditional children, probabilistic cells and geometry-aware encounters/loot. Migrate giant tree, timber grove and snake pit away from unconditional entrance kits and repeated reward triplets. Holding the landmark footprint fixed must still yield different tactical arrangements. Retain intentional signature rewards; protect required progression, while optional loot may require equipment found elsewhere.
- [ ] **GEN-GROWTH:** Add deterministic passes that originate at a scene and extend beyond its reservation using branching paths and rasterized lines/polygons. Define material conversion/carving/merging rules, termination, overlap priorities and bounded work. Replace blanket landmark exclusion with specific protected entrances/objectives and compatible later layers; preserve outer boundary and required route/lock rules without fencing every scene into its original room.
- [ ] **GEN-SPIDER-SPREAD:** Spider scenes can grow branching tunnels or polygonal chambers into adjacent terrain, with variable reaches/widths and nested web/brood/remains rolls in the resulting spaces. Population responds to the grown geometry; growth is not an identical attached corridor or an obligatory feature of every spider room.
- [ ] **GEN-ROOT-SPREAD:** Giant trees can send lightning-like branching roots across room boundaries. Roll branch topology, reach and thickness; convert eligible contacted non-wood terrain to actual wood/root material so cutting/burning changes routes. Keep conversion visible, bounded and compatible with protected progression; annotate branches and rejected conversions.
...
- [ ] **GEN-FLOOR-PACING:** Explicit Forest stage profiles: 1-1 simpler/friendlier, fewer simple enemies (moths, occasional zombie, bats, mosquitoes, chickens), streams and lower complexity; 1-2 introduces more chaotic combinations; 1-3 opens the full Forest interaction/encounter mix; 1-4 includes a boss and distinctive Ice-border transition content. Tune components, growth, hazards, density, rewards and enemies together, not just HP/count multipliers. Coordinate existing boss and biome-transition tasks; expose profile weights/windows in the inspector and verify ordinary generation coverage.
