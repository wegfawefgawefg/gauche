...
- [ ] **FALL-4:** Specify fall-direction policy for each tall scenery family: away from the blow, toward it, fixed structural facing/load, or one saved random choice. Lean/art and warning lane must communicate the committed result; repeated hits cannot reroll it. Streetlights and Ice pillars use away-from-the-breaking-blow; review future trees, loaded supports and towers individually.
...
- [ ] **DRESS-2:** Add wire-bound rock stacks, wooden pallets and pallet stacks, and shipping containers. Place walk-through containers and small container clusters using overhead reveal, with clear entrances, contents and appropriate material destruction; preserve routes.
...
- [ ] Audit every enemy's notice/windup/strike/recovery, attack effects and first-hit counterplay; prioritize Wolf (currently reads as a lean and thunk), snail and legacy circle/gun enemies. User confirms the revised zombie windup plus slash is an excellent readability benchmark. Give each attack a readable species-appropriate tell, visible strike at actual impact, fitting sound and recovery; a timer or subtle lean alone is insufficient. Rework inherited art/behavior; compare current Stoker to the reported older build.
...
- [ ] **COOK-HEAT:** Let suitable ordinary fires cook food, not just campfire fixtures. Reuse portion, inventory-capacity and sizzle rules; sustained heat should cook over time rather than any brief fire-damage hit instantly cooking a meal. Audit burning surfaces/props and loose versus held food, extinguishing, and overcooking behavior before expanding recipes.
