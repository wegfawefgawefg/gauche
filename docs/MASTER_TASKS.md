...
- [ ] **ARROW-CONTACT:** Moving targets can step through travelling arrows between projectile beats. Check head-on crossing and perpendicular entry in both entity orders; resolve actual tile contacts once, preserving piercing, parries, ownership, fire and deterministic replay.
- [ ] **MIMIC-TELL:** Make the existing delayed bite visible: lid/jaw windup, committed bite effect and recovery; check approach, reveal, damage/displacement interrupts and deceptive box state. A sound alone is not adequate.
...
- [ ] **ICE-VARIETY:** Vary tall ice-pillar height/shape and add abundant smaller icicles, spikes and snow piles in suitable ordinary terrain. Steppable brittle spikes break for about 1 damage and short bleeding, with readable density and safe approaches. Distinguish ordinary Ice features from actual biome-transition elements; frozen logs belong only on Ice 1 (2-1).
...
- [ ] **DEPTH-ORDER:** Roofs, tall props and actors must share ground-anchor Y ordering. Fix the frozen-log roof/ice-pillar overlap from the latest screenshot; check front/behind, both orientations, falling poses and inside-group reveal. Roofs cannot simply draw last.
- [ ] **GOLD-HOOK:** Fishing Line must retrieve real gold pickups as well as items; preserve exact value, pickup ownership, occupied-bank behavior and co-op state. Reproduce on an ordinary reachable pile.
...
- [ ] **GEN-LAYERS:** Audit each ordinary biome's actual generation order, fixed-coordinate sets, repeated shapes, density losses and orientation support. Refactor incrementally around room terrain, optional nested component groups, inhabitants/props/item sockets, cross-room passes and whole-floor overlays. Seeded weighted alternatives, quantities, footprints, conditional child components and empty outcomes must compose; keep coherent authored exceptions, protected routes and independent scene budgets. Measure coverage/repetition in ordinary seeds rather than counting catalog entries.
- [ ] **GEN-ORIENT:** Give component groups explicit quarter-turn/mirror transforms, applying them consistently to terrain, facing, fixtures, connections, sockets and art. Audit rectangular footprints/entrances under all legal orientations; per-sprite UV rotation alone is insufficient.
- [ ] **BEAR-ROAM:** Ordinary roaming bears and lone oversized-bear clearings, with distinct readable silhouette/threat/reward and variable remains. Larger art can remain single-cell mechanically; do not require a multi-cell actor engine.
- [ ] **BEAR-FISH:** Restore visible Forest wall springs/streams and a bear fishing/eating beside them. Use real finite fish, recoverable/interceptable food, interrupted work and ordinary territorial combat; optional skeletons, gnawed bones, rotten fish and bank clutter in varied arrangements.
- [ ] **BEAR-FAMILY:** Crowded sleeping-bear den variants with cubs, mothers/fathers, varying group sizes, actual beds and optional remains/cache components. Quiet approach, noise waking and family responses must be readable. Separate single-room dens from larger connected habitats; one fixed two-room scene does not complete this family.
- [ ] **FOREST-SPRINGS:** Audit why entire Forest runs can lack the previous wall-source streams. Record planned Brook rooms, suitable source cells, successful placements and failures; restore ordinary varied source/pool/channel placements without putting every source in the same corner.
- [ ] **ICE-SETS:** Replace the repeated two-hole placement scan with seeded suitable-site selection, varying counts/spacing/bank side. Review related fishing/reservoir terrain and empty interiors for nested prop/population/loot components; preserve dry routes and valid diver habitats.

Concrete Forest landmarks (normal generation, not debug-only catalog entries):

- [x] **FOREST-DEN:** Reserve a connected pair of rooms for a bear sleeping hollow and stream approach: broad shared geometry, dry through-route, clustered beds/sleepers, food/remains and a guarded native-tool cache. Keep independent scene population, prevent ordinary scatter overwriting it, and measure normal frequency. First connected hollow variant now generates with rolled banks, 2–6 sleeping bears, nest sites, remains/vegetation clusters and a native tool cache. 45 placements/128 ordinary floors; repeatability, dry bed/cache access, routes/locks, noise waking, snapshots and both-orientation static captures passed. This does not complete BEAR-ROAM/FISH/FAMILY or the larger 2x2 family.
- [ ] **FOREST-SPIDER:** Reserve a 1x2/1x3 spider cave with burnable/cuttable webs, hatchlings/adults, optional mother chamber and a meaningful cache. Supply multiple ways through; no required exit behind an unsolvable web.
- [ ] **FOREST-SNAKE:** Curved 1x2/1x3 snake tunnel/pit, visible holes and snakes, ledges/safe approaches and supplies worth entering for. Integrate real pit/displacement rules; do not just rename a Den room.
- [ ] **FOREST-ROOTS:** A cross-room fallen-root maze with axe/fire shortcuts, entrances and overhead reveal; one reserved structure across existing room boundaries.
- [ ] **FOREST-GIANT:** Rare 2x2 hollow giant tree with root entrances, grouped overhead reveal and an inhabited inner cache.
- [ ] **FOREST-TIMBER:** Connected 2x2 dense timber grove with burnable undergrowth, deliberate firebreaks and bounded fire-spread cost; distinct from the eventual whole-floor Timber Forest.
...
- [ ] **ICE-ARCH:** Replace the cylindrical design with open, angular ice ribs: two grounded tapering buttresses, a faceted bridge crown and hanging icicles, with empty air visible underneath. Vary height/span; no repeated log cross-section. Walk-under reveal and melting must match its footprint. Ordinary Ice 2–4 placement plus rarer Ice 1, separately from frozen transition timber.
