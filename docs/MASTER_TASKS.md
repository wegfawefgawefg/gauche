...
- [ ] Dog readability feedback: replace the floating-head silhouette with a code-drawn animal; replace instant adjacent bite with committed windup, visible bite and recovery. Identify the second obscured silhouette from the same playtest screenshot before changing its behavior.
