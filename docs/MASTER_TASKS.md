...
- [ ] **FOREST-COMBAT-DENSITY:** Preserve the recent combat-heavy pass while recomposing generation. Make all four floors lively with things to fight, break and use; tune difficulty through enemy mix/telegraphs/terrain as well as numbers. Inspect enemy and breakable density per traversable sector, including newly carved open ground, so landmark reservations and growth do not silently hollow out ordinary encounters. Count ordinary hostiles separately from passive life and conditional threats; decorative clutter does not satisfy combat density. Keep 1-1 busy with readable simpler enemies, and build denser mixed packs and overlapping threats on later floors. Compare against the pinned baseline and user playtesting; Gauntlet/Qud is the direction.
  ...
  - Added bounded outlying encounters after ordinary/cross-room populations: reachable quiet patches independently roll empty outcomes, packs or terrain-appropriate specialists; entrances/objectives, reserved habitats and bears stay excluded. Later ordinary clearings/thickets/ruins have broader specialist/source pools. Inspector records decisions and fine steps. Build, 32-floor capture/hash/codec/route audit, habitat/capacity/report-cap fixtures and two ordinary viewer previews passed. Same 64 seeds: late-floor specialists average 5/5 versus 3.8/2.4; enemy sources 1.2/1.6 versus 0.3/0.4, with basic fighter counts close to prior. Quiet-area reductions are modest; human combat and exposure tuning remain open.
...
- [x] **GEN-FRACTIONAL-ZOOM:** Fixed uncovered terrain pixels from SDL software rectangle truncation at fractional zoom. Flat tiles use a four-triangle fan; lit meshes keep opposite triangles apart so SDL preserves shared edges. Texture rotation stays in UVs, including log floors/springs; chasm fills and lava use the same path. Build, actual-viewer opaque-coverage checks at five zooms and three renderer scales, four spring orientations, UV crop/flip/tint/opacity comparisons, and three ordinary Forest lit/full-bright previews passed. These are software-renderer checks; hardware backend appearance remains for live playtesting. No gameplay or generation changes.
- [x] **GEN-CONTROLLER-PLAY:** Main-menu Dev option opens World Gen. Controller bumpers switch Forest floors, a labeled action regenerates, sticks/pad pan and zoom, and fit/annotation controls remain accessible without a mouse. Always display seed; expose copyable seed/revision/settings for feedback. Play enters the exact inspected generation, not a new roll; return/replay retains its original state separately from play mutations. Inspecting must not advance enemies or hazards. Implemented main-menu viewer, physical controller/keyboard controls, exact completed-floor Play and original-state return/replay. Strict build, four-floor capture-on/off hashes and route access, direct controller actions, exact copy/replay checks and dummy-driver static menu/viewer captures passed; user live playtesting remains.
  - Added native results sidebar with controller/keyboard feature and child-roll browsing, actual checkpoint outcomes/counts, map focus and independent map/list controls. Fit/focus reserve panel space; fine-step following synchronizes the selected feature, and earlier checkpoints clear invalid child selection. Back/Tab browses, A focuses, B returns to map; map A still plays. Build, four-floor fit bounds, direct controller/keyboard/repeat/floor-switch checks, original-state hashes and feature/child static previews passed. ImGui retains detailed weights/eligibility.
...
- [ ] **GEN-THEME-MIX:** Add weighted major floor identities, compatible minor modifiers and occasional combinations, including ordinary Forest with no dominant event. Major themes influence terrain, resources, populations and repeated physical clues; ants must be recognizable beyond one isolated ant room. Tune per-stage/per-run exposure explicitly, explain eligibility and roll outcomes in inspector, and measure actual placements/rejections. Initial proposal: usually zero/one dominant theme plus compatible minor features, with deliberate richer overlaps; this is tunable, not a universal hard cap. A 10% per-run independent chance gives ~65% exposure across ten runs, not certainty; distinguish per-floor rolls.
  - Initial theme layer implemented: stage-weighted None/Wet woods/Timber/Spider major identities and compatible None/Mushrooms/Ruins/Undergrowth modifiers. Shared registry exposes weights/compatibility; real room-role, landmark/river/sector chances, polygon terrain/scenery patches, tree opportunities and fighter families follow the selection. Nonblocking patches can overlap landmark ground; structural edits preserve objectives, roofs, hazards and hidden contents. Separate terrain/scenery checkpoints and recorded child patches expose effects/failures. 32-floor sample: 15 no major, 9 wet, 3 timber, 5 spider; all minor outcomes, 241 patch attempts, 6,795 recorded changed cells and 767 recorded ordinary fighters. Route/lock/codec/capture/report-cap checks and static previews passed. Ant and mushroom identities are integrated below; richer mixed ecology, exposure balancing and user acceptance remain open.
- [ ] **GEN-ELIGIBILITY-PANEL:** Eligibility/rule details may live in ImGui; make actual session reports available during ordinary play/co-op/quickplay, not only in the viewer. Six large Forest landmark rules now drive both rolls and the inspector: floor-chance matrix, pending/ineligible/missed/failed/reserved/built/unique-suppressed outcomes, candidate counts, variants, footprint focus and reservation-pass links. Reports persist through ordinary play and now travel as bounded developer attachments on join/resync snapshots. Ordinary room/component coverage and later removal attribution remain.  Controller-accessible sidebar lists registered themes/features/components for the selected biome-stage and their actual generation results. Show placed (count/variant), eligible but not selected, ineligible (floor/profile/prerequisite reason), suppressed by conflict/budget, selected but placement failed, and pending at the selected timeline checkpoint. Distinguish rolled selection from successful placement and later removal/overwriting. Show authored weights/chances with their pool/roll scope, never label a weight as an unconditional spawn percentage. Floor switching exposes how eligibility changes across 1-1–1-4; filters for present/eligible/all and a compact floor-eligibility matrix help identify missing coverage. Expand an entry for dependencies, exclusion/placement reasons and child rolls; selecting a placed entry highlights its footprint or focuses its timeline event. Use the same registration/rule evaluation and captured decisions as generation, not a separately maintained UI list or new random draws. Unimplemented ideas are not presented as rollable content.
  - Ordinary encounter eligibility now shares pure stage/role/theme pools with generation: spacing, requested sizes, all families and their member pools, specialists and wildlife. ImGui previews unrolled alternatives, zero weights and exclusion reasons without changing the world; mismatched host revisions keep recorded choices authoritative. Build, 32 identical gameplay hashes, 1,922 captured rows matched to the shared pools, codec/route checks and three UI previews passed. No combat weights changed.
  - Live inspector now shares footprint/guide/cell/rejection annotations with the viewer, using the gameplay camera and independent selections cleared on report replacement/loss. Overlay is opt-in and can stay visible with F1 closed; floor diagnostics are copyable. Fine-scope dropdown comes from registered instrumentation and now includes timber, snake and outlying passes previously inaccessible there. Build, pixel parity across cameras/zooms, renderer-state and game-hash neutrality, report lifecycle checks and actual live static previews passed; 64-floor audit unchanged. Remote diagnostic transport is implemented below.
  - Developer network snapshots now attach the host generation report/revision through existing paced, checksummed snapshot delivery. Gameplay hashes/plain snapshots remain unchanged; validated geometry, child choices and outcomes survive join/resync. Attachment cap is 256 KiB with explicit metadata-only fallback; malformed diagnostic contents cannot reject valid gameplay state. Build, 32-floor exact report/codec/route checks, malformed/envelope/cap fixtures and real localhost join/resync at tick zero passed. Largest sampled attachment 106,025 bytes; no omitted geometry. Network content version advanced; remote WAN playtesting remains user-owned.
- [ ] **FOREST-ANTS:** Small/medium/large ants with readable roles and trails from a finite sugar pile to a colony destination. Workers actually gather/carry/deliver; large ants carry conspicuous cargo, medium teams pull heavy cargo with visible ropes. Moving loads can crush occupants with a readable warning and escape opportunity. Whistle-led teams coordinate, respond to blocked paths/interference and use generation-qualified leaders/cargo. Include colony-heavy floor themes with scattered evidence, route/resource variation and mixed-theme opportunities; use appropriately simple body/collision ownership rather than requiring generic multi-tile actors.
  - Initial colonies implemented: finite sugar sources, actual worker/porter round trips, nest deliveries, whistle work boosts and provoked colony defense with committed bite windups. Generation-qualified links survive snapshots; destroyed nests/sources affect jobs. Ordinary Forest placement and Ant trails major theme add colonies after all fighter populations; later blocking scenery preserves their trails. Code-generated art; build, direct behavior/snapshot checks and 32-floor route/lock/capture audit passed. Two-ant rope crews now share those finite sources, haul bundled loads along wide routes, and stop when a puller is disabled, missing, displaced or defending the colony. A full-second marked pull deals 48 damage and shoves; trapping a victim behind it can crush. Linked whistle captains shorten recovery, never warning time. Build, detour/delivery/conservation, interruption/collision, snapshot, bounded generation and static render checks passed. Exposure/traffic tuning, richer leader orders and human combat acceptance remain open.
- [ ] **FOREST-MUSHROOM-GNOMES:** Mushroom forests grow as variable terrain/vegetation regions with small houses and tiny gnomes. Gnomes flee/scream at an approaching player, enter real houses and shut doors; attacks on their homes can provoke a coordinated armed response. Add readable stick/bow/crossbow roles and gnomes riding ants, with clear affiliation, response scope, population bounds and persistent ownership. Settlement layouts, occupied/empty houses, resources and adjacent ant activity compose stochastically, not one fixed army scene. Houses and terrain support normal damage/fire/route interactions.
  - Initial ordinary-generation implementation: irregular fungal districts independently roll wild growth, lone homes/hamlets, finite households, defender roles and native cupboards. Gnomes flee through actual front doors, hide, and emerge for settlement-scoped retaliation or burning/destroyed homes. Stick/bow/crossbow and mounted ant riders have committed windups; ranged defenders expend finite arrows. Tall mushrooms provide breakable spore cover. New major identity, feature report and pass checkpoint; code-generated art/sounds. Build, focused behavior/ownership/snapshot checks, 32-floor capture/route/lock audit and static previews passed. That sample retains 795 ordinary pack members, with households added afterward. Exposure, mixed ecology, combat feel and user acceptance remain open; riders currently use one composite body.
- [x] **GEN-ROOM-ASSIGNMENTS:** Inspect Forest base roles followed by required objectives, secret leaves, landmark reservations/additions, socket rotations and geometry-validation reversals. Record actual before/after state without RNG; distinguish assignments from weighted choices in both native sidebar and ImGui. Developer report codec carries these records and accepts older weighted reports. Build, 32-floor final-role/footprint/parent-chain checks, bounded-report validation and three ordinary UI views passed; all 32 gameplay hashes matched the preceding revision. Later prop/actor removal attribution remains open.
- [ ] **GEN-ANNOTATIONS:** Adapt Splonks's transient debug rectangles/labels (`src/state.hpp`, `state.cpp`, `render/debug.cpp`) and persistent stage-generation annotations; add world-space lines, polylines, polygons and point/cross markers. Generation records survive until regeneration and are redrawn per frame; runtime annotations clear/rebuild per simulation frame. Filter by pass/component/type; show room and child variants, footprints, chosen rolls/weights, attempted spawn sites and an X for empty/rejected outcomes with reason. Capture existing decisions without additional gameplay RNG calls or debug-dependent hashes.
  - Added shared preview/live layer controls for guides, areas, sites, rejections and labels; selected-child/feature/all-component scopes and room role/rotation labels. Labels are bounded and skip overlaps. Focused pixel/filter/state-neutrality checks and ordinary static renders passed. Uninstrumented operations and transient runtime annotations remain open.
  - Recorded terrain/fuel areas now use faint tint and exposed-edge outlines; individual spawn/prop/item sites retain crosses. Generator-owned area/site metadata survives developer network reports (v3; v1/v2 remain readable), and area counts deduplicate overlapping strokes. Build, 32 unchanged gameplay hashes, report compatibility/validation, fractional-zoom pixels and ordinary viewer previews passed. Runtime annotation facilities and broader filtering remain open.
- [ ] **GEN-PASS-VIEW:** Inspect base terrain, room components, cross-room growth, infrastructure, population and final scatter separately; show river/root guide paths and rasterized footprints, parent/child component identity, affected materials, reserved routes and rejection causes. Support holding layout fixed while rerolling child components/population, with explicit reproducible seed controls. Record only when inspection is enabled; bound trace memory and avoid annotation clutter by default.
  - Added optional inhabitants seed after boss geometry, leaving earlier checkpoints fixed while rerolling inhabitants and later dressing; default zero preserves the planner stream. N/LT+X, explicit seed/restore controls, copied recipes and developer report v4 retain the variant. Later scenery may alter terrain; uniques ignore the override. Build, 64 variants/1,024 matching early checkpoints, routes/locks, fine-capture neutrality, snapshot/legacy reports and exact controller Play/return/recapture checks passed. Arbitrary individual-child rerolls remain open.
- [x] **GEN-TIMELINE:** Forward/backward stepping, slider/list, first/final controls and camera-preserving same-map recapture are implemented. Optional fine capture resolves sector attempts, river attempts and spider/root branches, including rejection/rollback, with feature filtering and every-N sampling. Fine steps follow their recorded component and retain matching reports/map state. Terrain/prop/current changes and actor/loot additions/removals/moves can be highlighted. Separate caps retain up to 36 coarse and 32 fine snapshots, with approximately 48 MiB additional fine storage; limits are visible and Play always uses the retained completed map. 32-floor fine-on/off checks matched every coarse checkpoint and final hash (128 fine snapshots, 35 empty/rejected attempts), plus filtering/sampling/cap, camera, same-map recipe, controller stepping and static UI checks. Other loops can opt into the shared capture mechanism; no mutation undo or simulation is used.
- [ ] **GEN-COMPOSE:** First nested component layer delivered for giant-tree/grove/snake supplies: independent interior pockets, child supply/prop rolls, native master-table rewards, optional island prizes with exact placement, and recorded empty/failure outcomes. Removes repeated entrance bundles and the guaranteed axe beside the snake crossing. Geometry/population composition now covers these three landmarks as detailed below; broader ordinary-room migration and human variety acceptance remain open.  Extend GEN-LAYERS with room/landmark alternatives containing smaller weighted component slots, conditional children, probabilistic cells and geometry-aware encounters/loot. Migrate giant tree, timber grove and snake pit away from unconditional entrance kits and repeated reward triplets. Holding the landmark footprint fixed must still yield different tactical arrangements. Retain intentional signature rewards; protect required progression, while optional loot may require equipment found elsewhere.
  - Giant-tree interiors now roll open hearts, root galleries, split hearts, wet hollows or rotten floors, then independently compose crooked root partitions, openings and irregular pools/gaps. Required dry routes/cache approaches survive; real breakable lights and separately rolled fighter groups replace fixed lighting and the old binary population. Ten ordinary viewer seeds produced ten distinct interiors and 14–26 direct fighters each; fine capture, routes/locks, snapshot and report neutrality passed. A 32-floor regression and full-bright/lit static views passed.
  - Timber groves now compose irregular outlines, 5–11 rolled glades, connected fuel/mineral trails, wall-backed springs, independent tree/understory/fuel rolls and separate fighter/visitor populations. Nine viewer seeds yielded distinct terrain, 6–24 trees and 20–29 direct fighters each; fine checkpoints, routes/cache/locks, capture neutrality and codecs passed. Felling all trees in all four directions retained bare routes/cache access. The 32-floor regression and full-bright/lit previews passed. Open-sector/river/spider carving now preserves spring support walls. A combined floor exceeded 256 component records, so the bounded inspector limit is now 512 (303 records in this sample); fine snapshot limits remain unchanged. Live exposure/balance remains open.
  - Snake habitats now roll rift banks, braided crossings or sinkhole beds, with independent link modes/widths, transverse crossings, bank erosion, colony sizes, extra flyers and scenery. Third-room extensions may turn corners. Optional islands roll independently; a nearby tree is no longer guaranteed, and no axe is supplied. Twelve ordinary viewer samples had distinct layouts and 11–23 snakes; two further corner seeds, dry routes/cache/locks, isolated prizes/actual tree bridges, capture/snapshot neutrality and a 32-floor regression passed. Full-bright/lit views inspected. Exposure, balance and geometry beyond the current two/three-room footprints remain open.
...
- [ ] **GEN-RIVER-RIDES:** Add an explicitly authored circulating-current variant with a generated loop and actual rideable lily pads/logs; treat it separately from source-to-sink rivers. Boarding, carrying, dismounting, blockage and deterministic co-op ownership need real behavior. Deep-channel variants have standable banks and clearly readable lethal deep water for unprotected actors; integrate flotation/escape rules and floor suitability rather than silently applying a new rule to all existing water.
  - Initial riding milestone: stage 2–4 river rolls can produce procedurally routed closed currents with feeding side pockets; shared routing/carving preserves locks and required walking routes. Ordinary rivers independently roll code-drawn lily pads/drift logs, after fighters. Supports carry one player and one real loose item, allow walking dismount, pause at obstructions/control/stilled or frozen water, and release displaced/dead/offline passengers through generation-qualified links. Build, focused ownership/cargo/control/stale-handle/snapshot checks, generated full-circuit riding, forced carving rollback and 32-floor capture/route/lock/codec audit passed: three loops, 32 supports, all eight fourth-floor bosses. Three ordinary viewer scenes and aboard renders inspected. Further flotation gear and live balance remain open.
  - Deep rivers now roll shallow/pool/channel depth by stage (1-1 stays shallow), retaining wading fords and access to previously reachable surviving floor. Dark water with current strokes is lethal without flight, swimming or raft support; loose items sink locally. Existing cargo floats, cold flasks, bridges and fallen-tree bridges work with the new surface; legacy Water is unchanged. Depth/ford fine checkpoints and nested roll details are inspectable. Build, support/cargo/toss/thaw/bridge/snapshot fixtures, forced ford repairs, 32-floor capture/route/lock audit and three static viewer previews passed. The 64-floor audit retained approximately 35/48/66/72 ordinary fighters per stage. Human readability and riding balance remain open.
  - Personal flotation: selecting an intact Air Bladder supports the player in deep rivers and follows currents, occupying the weapon hand; stowing, dropping, throwing or spending its final push/cargo use removes that support. Sleep/stun do not puncture it. Existing cargo/pulse behavior remains; a native float ring marks support in water. Build, focused entry/selection/current/raft/hazard/co-op/snapshot checks and static lit preview passed. No encounter or generation budgets changed.
- [x] **GEN-ORPHAN-LOOT:** Late Forest pass rolls spaced empty/native-cache/workshop sockets and buried native supplies after scenery. Existing finds, reserved scenes, objectives, roofs, hazards and protected cells are excluded. Shared master-table eligibility/usefulness/weights drive recorded source → role → item decisions; no new import opportunity. Exact stack sizes and wall extraction retained. Build, 32-floor capture/state/report-codec/extraction checks and capacity/report-cap/non-Forest fixtures passed: 69 loose, 23 buried, 151 empty sockets; all 64 comparison floors retained combat counts, terrain access and interactive props. Three ordinary inspector views checked. Human supply-balance tuning remains.
- [ ] **GEN-FLOOR-PACING:** Explicit Forest stage profiles: 1-1 simpler/friendlier but lively and combat-rich, using readable simple enemies (moths, occasional zombie, bats, mosquitoes, chickens), streams and lower complexity; 1-2 introduces more chaotic combinations; 1-3 opens the full Forest interaction/encounter mix; 1-4 includes a boss and distinctive Ice-border transition content. Tune components, growth, hazards, density, rewards and enemies together, not just HP/count multipliers. Coordinate existing boss and biome-transition tasks; expose profile weights/windows in the inspector and verify ordinary generation coverage.
  - Initial 1-4 milestone: Old Growth, a 640-HP territorial bear in a rolled polygon clearing, with ordinary encounters retained. Committed maul/rush, normal-play directional ground tells, breakable cover, stone-collision recovery, crowd-control interruption and a carried native weapon/40-gold reward. No arena lock: the existing exit and digging escapes remain independent. Connected snowy exit approaches, frozen side pools, snow piles and snow-capped existing trees introduce Ice without importing its enemy/loot tables. Both features have stage eligibility, placement reports and separate checkpoints. Build, direct combat/reward/control/snapshot checks, a 32-floor generation audit (8/8 sampled 1-4 boss/border placements; routes/locks intact) and static lit/full-bright previews passed. The 64-floor pinned-baseline comparison is recorded above (16/16 sampled fourth-floor bosses). Broader exposure tuning, alternate bosses and human combat/visual acceptance remain open.
...
- [ ] **TREE-VARIETY:** Replace uniformly three-tile oak scenery with varied heights/silhouettes and ordinary singles, small groups, groves and rings. Add conifers, rare thick 2x2 redwoods and 3x3 giant-tree constructions, plus mushroom fairy rings. Fixed multi-tile scenery is allowed without multi-cell moving actors; heights, fall reaches and collision must agree. Use layered seeded passes with varying density rather than a tiny global tree cap.
  - Ordinary eligible rooms now independently roll open/single/scattered/close/ring patterns, counts and height mixtures, replacing the global 2–4-tree cap. Two- through five-cell oaks have matching fall damage, logs, bridge spans and warning shadows; original landmark trees retain their authored three-cell size. Inspector records placements/rejections and fine stand checkpoints. Build, 32-floor capture/report/snapshot checks (303 ordinary trees, all four heights), 168 directional bridge fixtures and static previews passed; all 64 comparison floors retain exact combat populations. Shared connectivity checks also reject bear-den logs that sealed a passage on Forest 1-2 seed 49. Conifers, thick redwoods, fairy rings and human density/visibility tuning remain open.
...
- [ ] **GEN-BEAR-DEN:** Connected dens now compose polygon bowls/shelter branches, bent banks, spring pools/side creeks/seeping banks, root ribs with openings, and beds on actual dry ground. Root margins follow the carved outline; graph sockets and existing ground connectivity survive. Independent families/adults/old bears, finite fish/fishers, active approach packs, native supplies and scenery remain. Inspector records nested geometry/population choices and fine terrain steps. Build, 32-floor routes/locks/capture/snapshot checks and 48 facing/reflection/capacity fixtures passed; ordinary previews checked. The 64-floor audit retains non-den floors and averages 35/48/69/75 mobile fighters across Forest stages, plus specialists. Human combat/variety acceptance and further tuning remain open.
...
- [x] **AMBIENT-INSPECT:** F1 sound inspector exposes live local sources, ranges, entry boundaries, labels, pending/consumed/cooldown/voice state and mode/selection filters. Mapgen derives a silent source preview from the selected checkpoint with no trigger history; global sources are labeled explicitly. Mute affects ambience only. Build, live/preview lifecycle isolation, unchanged gameplay/audio history and static preview/live renders passed. This is inspection, not an ambient authoring or playback-preview editor.
