...
- [ ] **OVER-4 (playtest):** Fix hollow-log roof alignment and end perspective: the upper/far end should show a closed roof dome, not another complete opening; openings should face down/left/right as appropriate. Inspect the reported one-tile offset in both orientations. Reveal the group only while the viewer occupies the actual passage footprint, never merely beside its side walls.
