...
- [ ] **REPAIR-WEAR:** Normalize physical equipment wear versus consumable charges. Stick's 3/18 is stored as uses, so Resin Glue currently cannot mend it; the same issue affects other physical tools. Move appropriate melee/digging equipment to repairable durability while keeping ammo, fuel, portions and actual ability charges distinct. Preserve per-action cost, modifiers and tool-lifetime budgets; audit HUD/inspect wording, glue target selection/feedback and old/debug loadouts. Show what glue will repair and make its useful targets clear; do not refill consumables as a repair side effect.
