...
- [ ] **GIANT-CANOPY-VISIBILITY:** Stepping outside the hollow tree restores an opaque crown over the player and a large surrounding area. Reconcile the raised canopy's screen footprint with its reveal boundary; trim/shape the crown around the actual structure, and retain local occlusion relief wherever it hides the player or immediate exit approach. Avoid a large circular screen-cover and abrupt concealment at the interior edge. Inspect approaches/departures on all sides, every canopy size/height, partial damage and separate co-op viewers; retain ground-anchor depth and gameplay-independent visibility. See the preserved 2026-09-16 feedback/screenshots.
