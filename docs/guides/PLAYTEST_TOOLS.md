...
River shape can now select a circulating river on Forest 1-2–1-4. Try **1-2 seeds 1, 11 or 14** in the normal viewer; select its Circulating route child to see the actual closed centerline and feeding water cells. Independent Rideable supports / Support type rolls place lily pads and drift logs on loops and ordinary rivers. Walk onto one to ride and walk off to dismount; it carries one player and one loose item, follows the current, and pauses at blockers or stopped/frozen water. Current channels remain shallow; lethal deep variants and faster current strengths are pending.
