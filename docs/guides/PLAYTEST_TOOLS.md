...
F1 → enable **Presentation** opens a separate window that compares **Terrain lighting** (flat tiles, old triangle interpolation, current bilinear) without changing propagation or tile geometry. **Canopy behavior** includes whole-tree fade (default), sliding trees aside, the old oval stipple, a smooth rounded rectangle, the full canopy, or hidden. Whole-tree modes use a selectable center/root reference, approach radius, and transition distance. Fade changes one opacity per sprite; slide moves the whole rectangle outside the viewport as you approach and restores it as you move away. Neither uses the screen mask. These comparison controls are session-only. The rounded-rectangle option retains its cached 160×90 mask. Generate unmasked canopy studies with `python3 tools/art/overhead.py --previews /tmp/teeming-canopy-studies`.
