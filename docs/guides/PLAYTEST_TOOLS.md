...
| Copy actual generation recipe | C | ImGui button |
| Reroll inhabitants and later passes | N | LT + X |
...
**Annotations** exposes separate carving-guide, affected-area, placement/empty-site, rejection and label toggles. Scope can be one child, its feature, or all recorded components. Room role/rotation labels are separate. Labels skip overlaps; select a child for priority and full details in Rolls. Guides are recorded proposals, not necessarily the final carved footprint. Live play has the same controls under **Generation inspector → Annotation layers**; omitted network geometry and uninstrumented operations cannot be displayed.

**Composition** holds the earlier layout through the boss-geometry checkpoint while rerolling inhabitants and every later pass. N / LT+X advances the inhabitants seed; zero restores the ordinary planner stream. Later trees/roof scenes/border passes can still change terrain. Whole-floor uniques ignore this override. Copy recipe includes both seeds, and Recapture/Play preserve the applied variant even if input fields have pending edits.

**F1 → Gameplay → Ambient sound inspector** works during play and mapgen; the Annotations tab also opens it. Toggle labels, hearing ranges, entry boundaries, mode filters and selected-source isolation. Cyan circles show near/far volume falloff; orange diamonds show the actual Manhattan entry distance. Global sounds have no local range. Live detail shows inside/pending/consumed state, cooldown and playing voice. Mapgen derives sources from the selected checkpoint without playback or trigger history; Focus source centers it. Mute affects ambient gain only, not effects or scheduling. F1 can hide the panel while overlays stay visible.
...
In F1 → Generation inspector, expand **Ordinary encounter eligibility (all pools)** and choose a preview room role. It shows potential spacing, sizes, families, every family’s members, specialists and wildlife for the current floor and themes—even choices that did not roll. Zero weights cannot roll; family exclusions explain stage/theme requirements. These are conditional pools for unreserved ordinary rooms, not whole-floor probabilities or landmark populations. Previewing a role does not alter generation. For a host with a different build revision, this local-rules preview is unavailable; the host’s recorded choices remain below.

**Ordinary tree stands** in the results list shows room pattern, count, height mixture and each attempted tree's reach. Select that feature for fine checkpoints. Forest 1-2 seed 6 has a broken ring near (57,34); 1-1 seed 2 has scattered trees near (35,33). These are normal generator rolls; changing the seed changes the composition. Tall ordinary oaks now fall/bridge 2–5 cells according to their visible height.
...
F1 → enable **Presentation** opens a separate window that compares **Terrain lighting** (flat tiles, old triangle interpolation, current bilinear) without changing propagation or tile geometry. **Canopy behavior** includes whole-tree fade, sliding trees aside, the old oval stipple, a smooth rounded rectangle (default), the full canopy, or hidden. Whole-tree modes use a selectable center/root reference, approach radius, and transition distance. Fade changes one opacity per sprite; slide moves the whole rectangle outside the viewport as you approach and restores it as you move away. Neither uses the screen mask. These comparison controls are session-only. The rounded-rectangle option retains its cached 160×90 mask. Generate unmasked canopy studies with `python3 tools/art/overhead.py --previews /tmp/teeming-canopy-studies`.

The parallax forest canopy is an unbounded decorative field, independent of room floors, walls, and map edges. Rendering visits only the camera neighborhood, including extra rows for upright crowns entering from below.

Post-GPU playtest defaults: bilinear lighting, rounded-rectangle canopy, upright trees, clear area 0.83, edge fade width 0.25, and all contact-shadow categories enabled.
