...
**Connected bear den** now exposes resident, stream, approach-pack, supply and floor-patch choices. **1-1 seed 1** has a sleeping family and separate zombie packs; **1-3 seed 1** has a working fishing bear. Inspect the actual placed cells and reductions from bear buffers/arrival distance. Den terrain itself still uses the existing connected hollow layout.
