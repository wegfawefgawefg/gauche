...
F1 → enable **Presentation** opens a separate window that compares **Terrain lighting** (flat tiles, old triangle interpolation, current bilinear) without changing propagation or tile geometry. **Canopy opening** compares the old oval stipple, a smooth rounded rectangle, the full canopy, or hidden. Upright orientation, clear-area size, and fade width are adjustable; these comparison controls are session-only. The new default uses upright code-generated trees and a cached 160×90 mask, composited with premultiplied alpha in one quad. Generate unmasked canopy studies with `python3 tools/art/overhead.py --previews /tmp/teeming-canopy-studies`.
