...
void apply_health_damage(Game& game, int slot, int damage, Cell attacker) {
    ...
    if (entity.health == 0) emit_sound(game, entity.kind == EntityKind::BellDiver ? SoundId::DiverDeath : entity.kind == EntityKind::FrostBat ? SoundId::FrostDeath : entity.kind == EntityKind::RimeSkater ? SoundId::SkaterBreak : entity.kind == EntityKind::WaspNest ? SoundId::NestBreak : entity.kind == EntityKind::CrateMimic ?
