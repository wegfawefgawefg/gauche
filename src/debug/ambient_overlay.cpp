...
void draw_ambient_annotations(tr::Renderer* renderer,const Game& game,ViewCamera camera,float zoom) {
    ...
    for(std::size_t i=0;i<state.audio->sources.size();++i) {
        ...
        const auto circle=[&](float radius) {
            ...
            }tr::lines(renderer,points,49);
        };
        tr::set_color_bytes(renderer,90,225,230,180);
        tr::line(renderer,x-3,y,x+3,y);tr::line(renderer,x,y-3,x,y+3);
        ...
        if(state.triggers && spec.mode==AmbientMode::Enter) {
            tr::set_color_bytes(renderer,source.consumed&&!state.preview ? 155 : 255,155,80,230);
        ...
        if(clear){labels.push_back(box);tr::set_color_bytes(renderer,10,16,20,225);tr::fill_rect(renderer,&box);
            tr::set_color_bytes(renderer,230,235,190,255);tr::debug_text(renderer,box.x+2,box.y+2,label);}
    }
    tr::set_color(renderer,r,g,b,a);tr::set_blend(renderer,blend);
}
