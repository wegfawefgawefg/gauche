...
namespace {
void area(tr::Renderer* renderer,std::span<const Cell> cells,ViewCamera camera,float zoom,SDL_Color color) {
    ...
    tr::set_color_bytes(renderer,color.r,color.g,color.b,255);
    ...
    for(Cell c:sorted) {
        ...
        if(!contains(c+Cell{-1,0}))tr::line(renderer,box.x,box.y,box.x,box.y+box.h);
        if(!contains(c+Cell{1,0}))tr::line(renderer,box.x+box.w,box.y,box.x+box.w,box.y+box.h);
        if(!contains(c+Cell{0,-1}))tr::line(renderer,box.x,box.y,box.x+box.w,box.y);
        if(!contains(c+Cell{0,1}))tr::line(renderer,box.x,box.y+box.h,box.x+box.w,box.y+box.h);
    }
}
}
...
void draw_generation_annotations(tr::Renderer* renderer,const GenerationReport& report,
    ...
    SDL_BlendMode old_blend;tr::get_blend(renderer,&old_blend);
    float red=0,green=0,blue=0,alpha=0;tr::get_color(renderer,&red,&green,&blue,&alpha);
    ...
    if (options.bounds && selected_feature>=0) {
        ...
        if (decision) for (const auto& region:decision->regions) {
            ...
            tr::set_color_bytes(renderer,255,230,90,focused ? 110 : 255);tr::rect(renderer,&box);
        }
    }
    ...
    for(std::size_t n=0;n<report.components.size();++n) {
        ...
        tr::set_color_bytes(renderer,selected.placed ? 90 : 255,selected.placed ? 255 : 100,210,255);
        ...
        tr::set_color_bytes(renderer,255,140,90,190);
        if(options.rejected) {
        ...
        for (std::size_t i=1;options.guides && !selected.guide.empty() && i<selected.guide.size()+(selected.guide_closed ? 1U : 0U);++i) {
            ...
            tr::line(renderer,a.x+offset,a.y+offset,b.x+offset,b.y+offset);
        }
    ...
    for(const auto& label:labels) {
        ...
        tr::set_color_bytes(renderer,150,210,190,190);
        tr::line(renderer,label.anchor.x,label.anchor.y,box.x,box.y+box.h*.5F);
    }
    float sx=1,sy=1;tr::get_scale(renderer,&sx,&sy);
    for(const auto& label:labels) {
        const auto& box=label.box;
        tr::set_color_bytes(renderer,10,16,20,245);tr::fill_rect(renderer,&box);
        tr::set_scale(renderer,sx*.5F,sy*.5F);
        tr::set_color_bytes(renderer,205,250,235,255);tr::debug_text(renderer,(box.x+2)*2,(box.y+2)*2,label.text.c_str());
        tr::debug_text(renderer,(box.x+2)*2,(box.y+7)*2,label.counts.c_str());
        tr::set_scale(renderer,sx,sy);
    }
    tr::set_color(renderer,red,green,blue,alpha);
