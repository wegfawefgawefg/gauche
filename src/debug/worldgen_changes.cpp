...
void draw_worldgen_changes(SDL_Renderer* renderer,const WorldGenViewer& v,const Game& before,const Game& after) {
    ...
    const auto mark=[&](Cell cell,bool cross) {
        const auto box=tile_rect(cell,v.render.camera,v.zoom);SDL_RenderRect(renderer,&box);
        if (cross) {
    ...
    for (std::size_t i=0;i<after.entities.size();++i) {
        ...
        if (existed && (!same || old.cell!=now.cell)) {SDL_SetRenderDrawColor(renderer,255,90,80,230);mark(old.cell,true);}
        if (exists && !same) {SDL_SetRenderDrawColor(renderer,100,255,130,230);mark(now.cell,false);}
        if (same && (old.cell!=now.cell || old.health!=now.health || old.sprite!=now.sprite ||
            ...
            SDL_SetRenderDrawColor(renderer,255,225,80,230);mark(now.cell,false);
            if (old.cell!=now.cell) {
                const auto a=tile_rect(old.cell,v.render.camera,v.zoom),b=tile_rect(now.cell,v.render.camera,v.zoom);
                SDL_RenderLine(renderer,a.x+a.w*.5F,a.y+a.h*.5F,b.x+b.w*.5F,b.y+b.h*.5F);
            }
        }
    }
}
