...
void draw_ant_ropes(SDL_Renderer* renderer,const Game& game,const Entity& load,ViewCamera camera,float zoom,const LightingCache& lighting) {
    ...
    for (const auto& ant:game.entities) {
        ...
        SDL_SetRenderDrawColorFloat(renderer,light.red*.74F,light.green*.64F,light.blue*.42F,1);
        SDL_RenderLine(renderer,x,y,(x+ex)*.5F,(y+ey)*.5F+slack);
        SDL_RenderLine(renderer,(x+ex)*.5F,(y+ey)*.5F+slack,ex,ey);
    }
    if (load.label_a==LoadWarn) {
        ...
        SDL_SetRenderDrawColorFloat(renderer,light.red,light.green*.55F,light.blue*.18F,1);
        SDL_RenderRect(renderer,&target);
        SDL_RenderLine(renderer,target.x,target.y,target.x+target.w,target.y+target.h);
        SDL_RenderLine(renderer,target.x+target.w,target.y,target.x,target.y+target.h);
    }
}
