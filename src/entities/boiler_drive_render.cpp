...
void draw_steam_drive(SDL_Renderer* renderer,const GameGraphics& graphics,const Game& game,
    ...
    SDL_Texture* texture=texture_for(graphics,prop.broken ? Sprite::SteamDriveBroken : running ? Sprite::SteamDrive : Sprite::SteamDriveOff);
    SDL_SetTextureColorModFloat(texture,light.red,light.green,light.blue);
    SDL_RenderTexture(renderer,texture,nullptr,&rect);SDL_SetTextureColorModFloat(texture,1,1,1);
    ...
    SDL_SetRenderDrawColorFloat(renderer,light.red*.52F,light.green*.52F,light.blue*.38F,1);
    SDL_RenderLine(renderer,rect.x+rect.w*.5F,rect.y+rect.h*.5F,
    ...
    for (int spoke=0;spoke<3;++spoke) {
        ...
        SDL_RenderLine(renderer,cx,cy,cx+std::cos(a)*pixel*3,cy+std::sin(a)*pixel*3);
    }
}
