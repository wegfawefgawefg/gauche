...
void draw_boiler_feeds(tr::Renderer* renderer,const GameGraphics& graphics,const Game& game,
    ...
    for (const auto& feed:game.boiler_feeds) {
        ...
        tr::Texture* texture=texture_for(graphics,Sprite::SpringIntake);
        tr::texture_color(texture,light.red,light.green,light.blue);
        tr::draw_texture(renderer,texture,nullptr,&intake);tr::texture_color(texture,1,1,1);
        ...
        for (int i=1;i<=2;++i) {
            ...
            if (flowing && (broken || pipe.hp<24)) {
                tr::set_color(renderer,color.red*.45F,color.green*.74F,color.blue*.84F,1);
            ...
            if (flowing) {
                tr::set_color(renderer,color.red*.4F,color.green*.66F,color.blue*.7F,1);
...
void draw_boiler_water(tr::Renderer* renderer,const GameGraphics& graphics,const Game& game,const Entity& tank,SDL_FRect rect,LightColor light) {
    ...
    if (game.stage.at_or_border(tank.cell).surface.fire_ticks>0) {
        tr::Texture* flame=texture_for(graphics,game.tick/6%2 ? Sprite::FlameA : Sprite::FlameB);
        SDL_FRect fire{rect.x+pixel*3,rect.y-pixel*2,pixel*10,pixel*15};
        tr::draw_texture(renderer,flame,nullptr,&fire);
    }
    SDL_FRect gauge{rect.x+rect.w+pixel,rect.y+pixel*3,pixel*2,pixel*9};
    tr::set_color(renderer,light.red*.12F,light.green*.16F,light.blue*.15F,1);tr::fill_rect(renderer,&gauge);
    gauge.h*=static_cast<float>(feed->water)/boiler_water_limit;gauge.y=rect.y+pixel*12-gauge.h;
    tr::set_color(renderer,light.red*.35F,light.green*.72F,light.blue*.85F,1);tr::fill_rect(renderer,&gauge);
    if (feed->dry_ticks==0) return;
    tr::set_color(renderer,light.red,light.green*.4F,light.blue*.12F,1);
    SDL_FRect warn{rect.x+pixel*6,rect.y-pixel*4,pixel*3,pixel*2};
    if (feed->dry_ticks/10%2==0) tr::fill_rect(renderer,&warn);
    char label[32];std::snprintf(label,sizeof(label),"DRY FIRE %.1fs",static_cast<double>(180-feed->dry_ticks)/60);
    float sx=1,sy=1;tr::get_scale(renderer,&sx,&sy);tr::set_scale(renderer,sx*.5F,sy*.5F);
    tr::debug_text(renderer,rect.x*2,(rect.y-pixel*9)*2,label);tr::set_scale(renderer,sx,sy);
}
