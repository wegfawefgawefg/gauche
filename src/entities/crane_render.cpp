    ...
    for (const Entity& crane:game.entities) {
        ...
        if (shadows) {
            ...
            if (crane.label_b==1 && worker && worker->kind==EntityKind::CraneOperator) {
                ...
                tr::set_color(renderer,tint.red*.25F,tint.green*.29F,tint.blue*.25F,1);
                tr::line(renderer,base.x+pixels*.5F,base.y+pixels*.65F,
                    stand.x+pixels*.8F,stand.y+pixels*.85F);
                tr::Texture* console=texture_for(graphics,Sprite::CraneConsole);
                tr::texture_color(console,tint.red,tint.green,tint.blue);
                tr::draw_texture(renderer,console,nullptr,&stand);tr::texture_color(console,1,1,1);
                tr::set_color_bytes(renderer,crane_operator_ready(game,crane) ? 101 : 189,114,56,220);
            ...
            tr::set_color_bytes(renderer,0,0,0,85);tr::fill_rect(renderer,&shade);
            if (locked) {
                tr::set_color_bytes(renderer,181,125,60,155);
                SDL_FRect mark{target.x+pixels*.1F,target.y+pixels*.1F,pixels*.8F,pixels*.8F};
                tr::rect(renderer,&mark);
            }
            continue;
        }
        ...
        tr::set_color(renderer,light.red*.52F,light.green*.42F,light.blue*.24F,1);
        tr::line(renderer,pivot.x,pivot.y,ground.x,ground.y-height-pixels*.35F);
        tr::line(renderer,pivot.x,pivot.y+2,ground.x,ground.y-height-pixels*.35F+2);
        tr::set_color(renderer,light.red*.7F,light.green*.72F,light.blue*.70F,1);
        tr::line(renderer,ground.x,ground.y-height-pixels*.35F,ground.x,ground.y-height);
        SDL_FRect head{ground.x-pixels*.45F,ground.y-height-pixels*.35F,pixels*.9F,pixels*.7F};
        tr::Texture* texture=texture_for(graphics,Sprite::CraneHead);
        tr::texture_color(texture,light.red,light.green,light.blue);
        tr::draw_texture(renderer,texture,nullptr,&head);tr::texture_color(texture,1,1,1);
        if (locked) {
            tr::set_color_bytes(renderer,218,165,72,220);
            SDL_FRect lamp{head.x+head.w*.45F,head.y+head.h*.5F,2,2};tr::fill_rect(renderer,&lamp);
        }
    }
}
