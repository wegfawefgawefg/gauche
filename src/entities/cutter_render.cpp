...
void draw_cutter_details(SDL_Renderer* renderer,const GameGraphics& graphics,const Entity& cutter,
    ...
    SDL_SetRenderDrawColorFloat(renderer,light.red*.6F,light.green*.63F,light.blue*.55F,1);
    for (int w=-1;w<=1;++w) SDL_RenderLine(renderer,cx-dy*pixel*static_cast<float>(w),cy+dx*pixel*static_cast<float>(w),
        cx+dx*rect.w*reach-dy*pixel*static_cast<float>(w),cy+dy*rect.h*reach+dx*pixel*static_cast<float>(w));
    SDL_Texture* teeth=texture_for(graphics,Sprite::CutterTeeth);
    SDL_SetTextureColorModFloat(teeth,light.red,light.green,light.blue);
    ...
    SDL_RenderTextureRotated(renderer,teeth,nullptr,&head,angle,nullptr,
        working && cutter.timer_a/4%2 ? SDL_FLIP_VERTICAL : SDL_FLIP_NONE);
    SDL_SetTextureColorModFloat(teeth,1,1,1);
