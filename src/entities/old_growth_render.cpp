...
void draw_old_growth_warning(tr::Renderer* renderer,const Game& game,ViewCamera camera,float zoom,const LightingCache& lighting) {
    ...
    for(const auto& bear:game.entities) {
        ...
        for(int i=0;i<attack.count;++i) {
            ...
            tr::set_color_bytes(renderer,203,141,76,static_cast<Uint8>(alpha/4));
            SDL_FRect inset{rect.x+p*2,rect.y+p*4,rect.w-p*4,rect.h-p*6};tr::fill_rect(renderer,&inset);
            tr::set_color_bytes(renderer,221,159,89,alpha);
            if(rushing) {
                ...
                tr::line(renderer,cx-dx-dy,cy-dy+dx,cx+dx,cy+dy);
                tr::line(renderer,cx-dx+dy,cy-dy-dx,cx+dx,cy+dy);
            } else for(int claw=0;claw<3;++claw)
                tr::line(renderer,rect.x+p*static_cast<float>(4+claw*3),rect.y+p*11,rect.x+p*static_cast<float>(6+claw*3),rect.y+p*5);
