...
void draw_pump_nozzle(SDL_Renderer* renderer,const Entity& actor,SDL_FRect rect,LightColor light) {
    ...
    SDL_SetRenderDrawColorFloat(renderer,light.red*.7F,light.green*.65F,light.blue*.5F,1);
    SDL_RenderLine(renderer,cx+dx*size*.2F,cy+dy*size*.2F,cx+dx*size*.6F,cy+dy*size*.6F);
    // A small tank stripe shows whether this is a water or fuel load even at rest.
    SDL_SetRenderDrawColorFloat(renderer,light.red*color.red,light.green*color.green,light.blue*color.blue,.95F);
    SDL_FRect gauge{rect.x+size*.3F,rect.y+size*.65F,size*.34F*static_cast<float>(actor.counter_b)/1800,size*.06F};
    SDL_RenderFillRect(renderer,&gauge);
    if (actor.label_a==PumpWarn) {
        ...
        SDL_RenderLine(renderer,tip.x,tip.y,tip.x+extent*progress,tip.y+extent);
    }
    ...
    SDL_SetRenderDrawColorFloat(renderer,light.red*color.red,light.green*color.green,light.blue*color.blue,fade*.9F);
    for (int lane=-1;lane<=1;++lane) {
        const float offset=static_cast<float>(lane)*size*.06F;
        SDL_RenderLine(renderer,cx+dx*size*.6F-dy*offset,cy+dy*size*.6F+dx*offset,
