...
Handle spawn_entity(Game& game, EntityKind kind, Cell cell) {
    for (int slot = 0; slot < max_entities; ++slot) {
        ...
        case EntityKind::Chicken: {
            ...
            entity.sprite = variant < 2 ? Sprite::Chick : (variant == 2 ? Sprite::Hen : Sprite::Rooster);
            entity.health = entity.max_health = variant < 2 ? 1 : (variant == 2 ? 3 : 30);
            entity.move_interval = variant < 2 ? 18 : (variant == 2 ? 30 : 42);
            entity.move_wait = static_cast<int>(random_u32(game) % static_cast<std::uint32_t>(entity.move_interval));
            entity.impassable = true;
            break;
        }
        case EntityKind::RailLayer:
            entity.facing = {-1, 0};
            entity.move_interval = 1;
            break;
        case EntityKind::Train:
