...
std::uint64_t game_hash(const Game& game) {
    ...
    for (const Tile& tile : game.stage.tiles) {
        ...
        for (auto ticks : {tile.surface.liquid_ticks, tile.surface.fire_ticks, tile.surface.smoke_ticks, tile.surface.sleep_ticks, tile.surface.scent_ticks, tile.surface.warmth_ticks, tile.surface.whiteout_ticks}) mix(hash, ticks);
