...
void default_binds(BindsProfile& profile) {
    ...
    bind(profile, GubsyButton::KB_V, Action::CompactDetails);
...
void register_game_bindings(GubsyRuntime& runtime) {
    ...
    schema.add_action(action_id(Action::CompactDetails), "Toggle Item Details", "Items");
    ...
    if (existing != nullptr &&
        ...
        if (old_controller || old_drop || missing_compare || missing_details || old_faces) {
            ...
            if (missing_details) {
                bind(migrated, GubsyButton::KB_V, Action::CompactDetails);
                bind(migrated, GubsyButton::GP_BACK, Action::CompactDetails);
            }
...
bool compact_details_button_down(GubsyRuntime& runtime) {
    return down(runtime, Action::CompactDetails);
}
