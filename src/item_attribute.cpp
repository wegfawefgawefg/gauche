...
bool item_accepts_attribute(ItemKind kind, ItemAttribute attribute) {
    ...
    case ItemAttribute::Big:
        return kind == ItemKind::ThornCaltrops || item_is_melee(kind) || (spec != nullptr && spec->pattern.blast_radius > 0) || kind == ItemKind::Bomb ||
               kind == ItemKind::RocketLauncher;
    case ItemAttribute::Long: return (weapon && kind != ItemKind::AcornMine) || kind == ItemKind::RopeHook || kind == ItemKind::SwapSeed;
