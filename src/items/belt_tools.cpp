...
constexpr RegionalItem shoe{"Brake Shoe", "Jam an adjacent belt run for 6s. Four shoes. A hit of 16 damage breaks the brake. Trains ignore it.",
...
bool use_belt_tool(Game& game,int slot,Cell direction) {
    ...
    return user.inventory.held()->kind==ItemKind::BeltCrank ? crank_belt(game,target) : brake_belt(game,target);
}
