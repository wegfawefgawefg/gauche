...
void draw_cooking(tr::Renderer* renderer,const GameGraphics& graphics,
    ...
    tr::Texture* texture=texture_for(graphics,item_sprite(player.ground_item));
    tr::texture_color(texture,light.red,light.green,light.blue);
    tr::draw_rotated(renderer,texture,nullptr,&food,player.counter_a/6%2==0 ? -12 : 12,nullptr,SDL_FLIP_NONE);
    ...
    tr::set_color(renderer,light.red*.22F,light.green*.18F,light.blue*.1F,1);
    tr::fill_rect(renderer,&bar);
    bar.w*=static_cast<float>(player.counter_a)/cooking_ticks;
    tr::set_color(renderer,light.red,light.green*(full ? .25F : .7F),light.blue*.22F,1);
    tr::fill_rect(renderer,&bar);
