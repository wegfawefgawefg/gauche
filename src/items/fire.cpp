...
void ignite_struck_actor(Game& game, int slot) {
    ...
    if (target.health <= 0 || (target.move_interval <= 0 && target.kind != EntityKind::RootTurret) ||
        target.kind == EntityKind::Ember || target.kind == EntityKind::Train ||
        target.kind == EntityKind::RailLayer) return;
    if ((wading_actor(target) || target.kind == EntityKind::RootTurret) && surface_wet(game.stage.at_or_border(target.cell))) return;
