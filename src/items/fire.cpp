...
void ignite_struck_actor(Game& game, int slot) {
    ...
    if (target.health <= 0 || (target.move_interval <= 0 && target.kind != EntityKind::RootTurret && target.kind != EntityKind::WaspNest && target.kind!=EntityKind::AntNest && target.kind!=EntityKind::AntSugar && target.kind!=EntityKind::AntLoad && target.kind!=EntityKind::GnomeHouse) ||
    ...
    if ((wading_actor(target) || target.kind == EntityKind::RootTurret || target.kind == EntityKind::WaspNest || target.kind==EntityKind::AntNest || target.kind==EntityKind::AntSugar || target.kind==EntityKind::AntLoad || target.kind==EntityKind::GnomeHouse) && surface_wet(game.stage.at_or_border(target.cell))) return;
