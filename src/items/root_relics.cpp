...
constexpr RegionalItem seed{"Blink Seed", "Throw to blink. Walls and bodies stop it. Landing hazards apply. Roots and sealed arenas hold you.",
...
const RegionalItem* forest_root_relic(ItemKind kind) {
    ...
    case ItemKind::BlinkSeed: return &seed;
...
bool launch_root_relic(Game& game, int owner_slot, const Item& item, Cell direction) {
    ...
    shot->label_a = static_cast<int>(drill ? ProjectileKind::Drill : ProjectileKind::Blink);
    ...
    shot->timer_a = drill ? 360 : pattern.maximum * 6 + 6;
    shot->timer_b = drill ? 12 : 6;
