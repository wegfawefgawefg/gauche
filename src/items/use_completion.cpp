...
void finish_item_use(Game& game, Entity& user, Item& item, ItemKind used_kind, Cell target,
    ...
    if (item.max_uses > 0 && --item.uses <= 0) {
        ...
        else if (used_kind == ItemKind::InsulatedBoots) emit_sound(game,SoundId::InsulatorSpent,user.cell);
        else if (used_kind == ItemKind::RailSwitchKey) emit_sound(game,SoundId::SwitchKeySpent,user.cell);
