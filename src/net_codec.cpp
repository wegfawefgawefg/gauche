...
bool decode_game(std::span<const std::uint8_t> bytes, Game& game, std::string& error) {
    ...
    for (Tile& tile : result.stage.tiles) {
        ...
        if (tile.prop.kind >= PropKind::Count ||
            ...
            tile.prop.growth_ticks > (tile.prop.kind == PropKind::PayCage ? 60000 : tile.prop.kind == PropKind::Conveyor ? 360 : tile.prop.kind == PropKind::GroundingSpike ? 180 : tile.prop.kind == PropKind::Stove ? stove_fuel_limit : tile.prop.kind == PropKind::Candle ? candle_fuel_ticks : tile.prop.kind == PropKind::IceBlock ? 600 : tile.prop.kind == PropKind::AlarmClock ? 480 : 180) ||
            (tile.prop.kind != PropKind::PayCage && tile.prop.kind != PropKind::TensionSpring && tile.prop.kind != PropKind::Conveyor && tile.prop.kind != PropKind::GroundingSpike && tile.prop.kind != PropKind::Stove && tile.prop.kind != PropKind::Candle && tile.prop.kind != PropKind::Shoot && tile.prop.kind != PropKind::IceBlock && tile.prop.kind != PropKind::AlarmClock && tile.prop.growth_ticks != 0)) reader.okay = false;
