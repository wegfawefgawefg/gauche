...
float hit_angle(const Game& game, Cell target, float fallback) {
    ...
    for (int index = 0; index < game.sound_count; ++index) {
        ...
        if (sound.sound != SoundId::ZombieScratch1 && sound.sound != SoundId::Punch1 &&
            ...
            sound.sound != SoundId::BoarHit && sound.sound != SoundId::WolfBite && sound.sound != SoundId::WolfSnap && sound.sound != SoundId::DogSnap &&
...
void observe_entity(Cosmetics& cosmetics, const Game& game, int slot) {
    ...
    if (entity.kind == EntityKind::None) {
        ...
        if (pose.seen && pose.health>0 && pose.kind==EntityKind::MagnetCrane) {
            ...
            scatter_material(cosmetics.debris,pose.cell,DebrisKind::ChainLink,3,seed+23);
        }
        pose = {};
        return;
    }
    ...
    if (same && (entity.attack_wait > pose.attack_wait ||
        ...
        if (entity.kind==EntityKind::ForestSpider && entity.use_flash==10 && entity.use_flash>pose.use_flash)
            spawn_forest_spider_bite(cosmetics,entity);
...
void observe_sound(Cosmetics& cosmetics, const SoundEvent& sound, Cell focus) {
    ...
    case SoundId::KilnDeath:
        ...
        break;
    case SoundId::CrawlerDeath:
        scatter_material(cosmetics.debris,sound.cell,DebrisKind::CopperCurl,4,seed);
        scatter_material(cosmetics.debris,sound.cell,DebrisKind::SteelWasher,3,seed+17);
        break;
    ...
    case SoundId::EffigyThaw:
        ...
        break;
    case SoundId::EffigyDeath:
        scatter_material(cosmetics.debris, sound.cell, DebrisKind::Twig, 6, seed);
        break;
    ...
    case SoundId::FruitSplat:
        scatter_material(cosmetics.debris, sound.cell, DebrisKind::SeedHusk, 4, seed);
        scatter_material(cosmetics.debris, sound.cell, DebrisKind::MushroomStem, 3, seed + 1);
        break;
    ...
    case SoundId::RabbitEscape:
        push_debris(cosmetics.debris, sound.cell, 1.5F, .065F);
        break;
