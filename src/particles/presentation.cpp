...
float hit_angle(const Game& game, Cell target, float fallback) {
    ...
    for (int index = 0; index < game.sound_count; ++index) {
        ...
        if (sound.sound != SoundId::ZombieScratch1 && sound.sound != SoundId::Punch1 &&
            ...
            sound.sound != SoundId::BatBite && sound.sound != SoundId::SkaterHit && sound.sound != SoundId::DiverStrike && sound.sound != SoundId::MimicBite) continue;
        closest = length;
        source = sound.cell;
    }
...
bool bleeds(EntityKind kind) {
    ...
    case EntityKind::BellDiver: case EntityKind::RimeSkater:
