...
void draw_conveyor(tr::Renderer* renderer,const GameGraphics& graphics,const Game& game,Cell cell,
    ...
    if (incoming!=Cell{}) {
        ...
        const auto color=[&](float r,float g,float b) {tr::set_color(renderer,r*light.red,g*light.green,b*light.blue,1);};
        color(.16F,.19F,.2F);tr::fill_rect(renderer,&rect);
        ...
        for (Cell direction:{incoming,outgoing}) {
            ...
            color(.29F,.33F,.34F);tr::fill_rect(renderer,&lane);
            ...
            for (int n=phase;n<8;n+=3) {
                ...
                color(.43F,.47F,.46F);
                tr::line(renderer,center.x+dx*along-dy*4*pixel,center.y+dy*along-dx*4*pixel,
        ...
        for (int side:{-1,1}) {
            ...
            tr::line(renderer,center.x+dx*4*pixel,center.y+dy*4*pixel,
    ...
    tr::draw_rotated(renderer,texture,nullptr,&rect,static_cast<double>(prop.variant&3U)*90,nullptr,SDL_FLIP_NONE);
