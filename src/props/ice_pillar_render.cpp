...
void draw_ice_pillar(SDL_Renderer* renderer,const GameGraphics& graphics,const Game& game,
    ...
    float dx=3*floor.w*scale*static_cast<float>(dir.x)*std::sin(theta);
    const float dy=3*floor.w*scale*(static_cast<float>(dir.y)*std::sin(theta)-std::cos(theta));
    ...
    SDL_Texture* texture=texture_for(graphics,prop.hp<18 || prop.growth_ticks ? Sprite::IcePillarCracked : Sprite::IcePillar);
