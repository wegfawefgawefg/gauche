...
void draw_props(SDL_Renderer* renderer, const GameGraphics& graphics, const Stage& stage,
    ...
    for (int y = std::max(0, center_y - radius_y); y < std::min(stage.height, center_y + radius_y); ++y)
        for (int x = std::max(0, center_x - radius_x); x < std::min(stage.width, center_x + radius_x); ++x) {
            ...
            const Sprite sprite = prop.kind==PropKind::SnowWindbreak ? (prop.variant==1 ? Sprite::SnowWallV : Sprite::SnowWallH) : prop.kind == PropKind::BridgePlank ? ((prop.variant&1U) ? Sprite::BridgePlankV : Sprite::BridgePlankH) : prop.kind == PropKind::GroundingSpike && prop.variant>0 ?
            ...
            SDL_RenderTextureRotated(renderer, texture, nullptr, &rect, 0, nullptr,
                (prop.kind == PropKind::SnowWindbreak || prop.kind == PropKind::BridgePlank || prop.kind == PropKind::Doorstop || prop.kind == PropKind::GroundingSpike || prop.kind == PropKind::SpiderStrand || prop.kind == PropKind::Candle || prop.kind == PropKind::Stove || prop.variant % 2 == 0) ? SDL_FLIP_NONE : SDL_FLIP_HORIZONTAL);
