...
void draw_tall_tree(tr::Renderer* renderer,const GameGraphics& graphics,const Game& game,
    ...
    const auto draw=[&](Sprite sprite,const SDL_FRect* source,SDL_FRect rect,SDL_FPoint anchor) {
        ...
        tr::texture_color(texture,light.red,light.green,light.blue);tr::texture_alpha_bytes(texture,obscures ? 100 : 255);
        tr::draw_rotated(renderer,texture,source,&rect,angle,&anchor,SDL_FLIP_NONE);
    ...
    if (tile.surface.fire_ticks>0) {
        tr::Texture* flame=texture_for(graphics,(game.tick/8)%2 ? Sprite::FlameA : Sprite::FlameB);
        for (int n=0;n<=tree_burn_height(tree)/4;++n) {
            ...
            tr::draw_texture(renderer,flame,nullptr,&rect);
        }
    }
}
void draw_tree_ground(tr::Renderer* renderer,const GameGraphics& graphics,const Game& game,
    ...
    if (prop.kind==PropKind::TallTree && !prop.broken) {
        ...
        tr::set_color_bytes(renderer,0,0,0,160);tr::fill_rect(renderer,&shadow);
        if (prop.growth_ticks) {
            ...
            for (int n=1;n<=tree_height(prop);++n) {
                ...
                tr::set_color_bytes(renderer,0,0,0,175);tr::fill_rect(renderer,&mark);
                tr::set_color_bytes(renderer,179,134,67,215);tr::rect(renderer,&mark);
            }
        }
        tr::set_blend(renderer,SDL_BLENDMODE_NONE);return;
    }
    const auto light=light_at_cell(lighting,cell);
    ...
    tr::draw_rotated(renderer,texture,nullptr,&floor,prop.kind!=PropKind::TallTree && (prop.variant&1U) ? 90 : 0,nullptr,SDL_FLIP_NONE);
