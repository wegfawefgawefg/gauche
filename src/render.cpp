...
void draw_entities(SDL_Renderer* renderer, const GameGraphics& graphics,
    ...
    for (std::size_t slot = 0; slot < game.entities.size(); ++slot) {
        ...
        SDL_Texture* texture = texture_for(graphics, entity.kind == EntityKind::GroundItem &&
            (entity.ground_item.kind == ItemKind::SteamKettle || entity.ground_item.kind==ItemKind::SteamLance || entity.ground_item.kind == ItemKind::HeatSiphon) ? item_sprite(entity.ground_item) : entity.sprite);
