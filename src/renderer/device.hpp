...
struct Stats {std::uint64_t vertices=0,triangles=0,batches=0,uploads=0;};
struct Backend {
    const char* name;
    bool (*init)(Renderer*);
...
void submit(Renderer*,Texture*,Texture*,SDL_BlendMode,const Vertex*,int,const int*,int);
...
Texture* create_texture(Renderer*,SDL_PixelFormat,SDL_TextureAccess,int,int);
...
bool draw_rotated(Renderer*,Texture*,const SDL_FRect*,const SDL_FRect*,double,const SDL_FPoint*,SDL_FlipMode);
bool raw_geometry(Renderer*,Texture*,const float*,int,const SDL_FColor*,int,const float*,int,int,const void*,int,int);
