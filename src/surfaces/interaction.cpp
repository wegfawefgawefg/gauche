...
bool dry_growth(const Prop& prop) {
    ...
    if (prop.kind==PropKind::BonePile || prop.kind==PropKind::BoundRocks || prop.kind==PropKind::ContainerSide) return false;
    if (prop.kind==PropKind::IcePillar || prop.kind==PropKind::IceRubble || prop.kind==PropKind::IceArchFoot || prop.kind==PropKind::IceSpikes || prop.kind==PropKind::SnowPile) return false;
