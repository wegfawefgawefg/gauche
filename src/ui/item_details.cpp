...
void draw_item_details(SDL_Renderer* renderer, const GameGraphics& graphics,
    ...
    if (item.kind==ItemKind::NozzleElbow || item.kind==ItemKind::PocketPump || item.kind==ItemKind::HeatSiphon || item.kind==ItemKind::StormLantern) std::snprintf(line,sizeof(line),"%s",item_state_text(item,false).c_str());
    ...
    if (item.kind == ItemKind::IceAnchor)
        std::snprintf(line,sizeof(line),"%s",item.anchor.slot>=0 ? "POINT PLACED" : "PLACE AT YOUR FEET");
    ...
    if (item.kind == ItemKind::Sled) {
        ...
        draw_action_hint(renderer,x+10,y+127,Action::Reload,item.loaded ? "LEFT / CHANGE BEND" : "RIGHT / CHANGE BEND");
    } else if (item.kind==ItemKind::PocketPump) {
        draw_action_hint(renderer,x+10,y+127,Action::Reload,"DISCHARGE AHEAD");
