...
void colored_cell(tr::Renderer* renderer, const PatternDiagramLayout& layout,
    ...
    tr::set_color_bytes(renderer, color.r, color.g, color.b, travel ? 76 : 178);
    if (!travel) tr::fill_rect(renderer, &cell);
    tr::set_color_bytes(renderer, color.r, color.g, color.b, travel ? 142 : 230);
    tr::rect(renderer, &cell);
}
...
void draw_pattern_diagram(tr::Renderer* renderer, const Item& item,
    ...
    for (int row = layout.min_y; row <= layout.max_y; ++row)
        for (int column = layout.min_x; column <= layout.max_x; ++column) {
            ...
            tr::set_color_bytes(renderer, 57, 68, 61, 72);
            tr::fill_rect(renderer, &cell);
            tr::set_color_bytes(renderer, 110, 126, 112, 80);
            tr::rect(renderer, &cell);
        }
    ...
    tr::set_color_bytes(renderer, marker.r, marker.g, marker.b, 255);
    tr::fill_rect(renderer, &player);
    if (layout.cell_size >= 6.0F) {
        const float scale = layout.cell_size / 8.0F;
        float old_x = 1.0F, old_y = 1.0F;
        tr::get_scale(renderer, &old_x, &old_y);
        tr::set_scale(renderer, old_x * scale, old_y * scale);
        tr::set_color_bytes(renderer, 18, 23, 20, 255);
        tr::debug_text(renderer, player.x / scale, player.y / scale, "P");
        tr::set_scale(renderer, old_x, old_y);
    } else {
