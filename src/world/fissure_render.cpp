...
void draw_fissures(tr::Renderer* renderer,const GameGraphics& graphics,const Game& game,
    ...
    for (const auto& f:game.fissures) for (int n=-1;n<=1;++n) {
        ...
        if (!plumes) {
            ...
            tr::texture_color(texture,std::max(light.red,glow),std::max(light.green,glow*(steam ? .9F : .65F)),std::max(light.blue,glow*(steam ? .8F : .35F)));
            tr::draw_rotated(renderer,texture,nullptr,&rect,f.axis.y ? 90 : 0,nullptr,SDL_FLIP_NONE);
        ...
        tr::texture_color(texture,std::max(.2F,light.red),std::max(.2F,light.green),std::max(.2F,light.blue));
        if (release && !steam) tr::texture_color(texture,1,1,1);
        tr::texture_alpha(texture,release ? .8F : pressure ? .4F : .24F);
