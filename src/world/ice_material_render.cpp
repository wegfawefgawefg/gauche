...
void draw_ice_material_details(tr::Renderer* renderer,const GameGraphics& graphics,
    ...
    for (Cell side:{Cell{1,0},{-1,0},{0,1},{0,-1}}) {
        ...
        tr::set_color(renderer,light.red*.32F,light.green*.43F,light.blue*.48F,1);
        for (int i=0;i<16;++i) {
            ...
            tr::fill_rect(renderer,&mark);
        }
    }
    if (tile.kind!=TileKind::Wall || tile.contents==ItemKind::None) return;
    tr::Texture* item=texture_for(graphics,item_sprite(tile.contents));
    const SDL_FRect inside{rect.x+rect.w*.22F,rect.y+rect.h*.2F,rect.w*.56F,rect.h*.56F};
    tr::texture_color(item,light.red*.65F,light.green*.83F,light.blue*.9F);
    tr::texture_alpha(item,.75F);
    tr::draw_texture(renderer,item,nullptr,&inside);
    tr::texture_alpha(item,1);tr::texture_color(item,1,1,1);
    ...
    tr::set_color(renderer,light.red*.38F,light.green*.56F,light.blue*.62F,1);
    tr::line(renderer,rect.x+pixel*4,rect.y+pixel*11,rect.x+pixel*11,rect.y+pixel*4);
    tr::line(renderer,rect.x+pixel*8,rect.y+pixel*10,rect.x+pixel*12,rect.y+pixel*6);
}
