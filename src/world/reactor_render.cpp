...
void draw_reactor_hazards(tr::Renderer* renderer,const GameGraphics& graphics,const Game& game,ViewCamera camera,float zoom) {
    ...
    for (int i=core->counter_b;i<core->counter_a;++i) {
        ...
        tr::set_color(renderer,.69F,.77F,.27F,pulse);
        // Small pulsing broken seam, not a solid debug-style attack square.
        tr::line(renderer,r.x+r.w*.15F,r.y+r.h*.72F,r.x+r.w*.46F,r.y+r.h*.5F);
        tr::line(renderer,r.x+r.w*.46F,r.y+r.h*.5F,r.x+r.w*.39F,r.y+r.h*.28F);
        tr::line(renderer,r.x+r.w*.39F,r.y+r.h*.28F,r.x+r.w*.8F,r.y+r.h*.14F);
    }
    const int rx=static_cast<int>(320/tile_pixels(zoom))+2,ry=static_cast<int>(180/tile_pixels(zoom))+2;
    tr::Texture* texture=texture_for(graphics,Sprite::ReactorFlame);
    for (int y=std::max(0,static_cast<int>(camera.y)-ry);y<std::min(game.stage.height,static_cast<int>(camera.y)+ry+1);++y)
        for (int x=std::max(0,static_cast<int>(camera.x)-rx);x<std::min(game.stage.width,static_cast<int>(camera.x)+rx+1);++x) {
            ...
            tr::texture_color(texture,.82F,.9F,.65F);
            tr::texture_alpha(texture,.9F);
            tr::draw_rotated(renderer,texture,nullptr,&flame,0,nullptr,seed%2 ? SDL_FLIP_HORIZONTAL : SDL_FLIP_NONE);
            tr::texture_color(texture,1,1,1);tr::texture_alpha(texture,1);
        }
...
void draw_reactor_status(tr::Renderer* renderer,const Game& game) {
    ...
    const auto banner=[&](float offset,SDL_FColor color) {
        ...
        constexpr int indices[]{0,1,2,0,2,3};tr::geometry(renderer,nullptr,vertices,4,indices,6);
    };
    tr::set_blend(renderer,SDL_BLENDMODE_BLEND);
    banner(3,{0,0,0,.7F});
