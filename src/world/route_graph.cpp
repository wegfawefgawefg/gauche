...
void describe_rooms(Game& game, FloorPlan& plan) {
    ...
    if (industrial_floor(game.run.floor)) {
        ...
        if (random_u32(game)%3!=0) for (std::size_t i=1;i<plan.rooms.size();++i) {
            ...
            const auto choice=random_u32(game)%5;
            room.role=choice==0 ? RoomRole::RepairBay : choice==1 ? RoomRole::CoolingWorks : choice==2 ? RoomRole::CableTrench : choice==3 ? RoomRole::KilnCourt : RoomRole::PayOffice;room.shape=RoomShape::Clearing;
        ...
        if (random_u32(game)%2==0) for (std::size_t i=1;i<plan.rooms.size();++i) {
            ...
            if (static_cast<int>(i)==plan.exit_room || static_cast<int>(i)==plan.objective_room ||
                ...
                room.role==RoomRole::AssemblyLine || room.role==RoomRole::RepairBay || room.role==RoomRole::CoolingWorks || room.role==RoomRole::CableTrench || room.role==RoomRole::KilnCourt || room.role==RoomRole::PayOffice) continue;
...
const char* room_name(RoomRole role) {
    constexpr const char* names[]{"Trailhead", "Way out", "Clearing", "Thicket", "Brook",
        ...
        "Reservoir", "Fishing hut", "Bathhouse", "Ice quarry", "Observatory", "Shelter", "Echo tunnel", "Weather station", "Cliff path", "Memorial court", "Candle chapel", "Crystal gallery", "Service passages", "Boiler gallery", "Work front", "Blasting alcove", "Assembly line", "Repair bay", "Scrap yard", "Cooling works", "Cable trench", "Kiln court", "Pay office"};
    static_assert(std::size(names) == static_cast<std::size_t>(RoomRole::PayOffice) + 1);
