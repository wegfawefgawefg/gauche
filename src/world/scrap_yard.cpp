...
Handle populate_scrap_yard(Game& game,const FloorPlan& plan,const RoomPlan& room) {
    ...
    // the crane's initial reach. This entire footprint avoids required paths.
    constexpr std::array offsets{Cell{4,-3},Cell{0,-3},Cell{-3,2},Cell{-2,-4},Cell{-2,1},
                                Cell{1,-3},Cell{2,-3},Cell{3,-3},Cell{2,-2}};
    for (int sign:{room.mirrored ? -1 : 1,room.mirrored ? 1 : -1}) {
        ...
        if (!okay) continue;
        const Handle crane=spawn_entity(game,EntityKind::MagnetCrane,at({4,-3}));
        if (!get_entity(game,crane)) return {};
        place_ground_item(game,at({0,-3}),ItemKind::Pickaxe);
        for (auto& e:game.entities) if (e.kind==EntityKind::GroundItem && e.cell==at({0,-3})) e.ground_item.durability=6;
        place_ground_item(game,at({-3,2}),ItemKind::HorseshoeMagnet);
        for (Cell offset:{Cell{2,-3},Cell{2,-2}}) *game.stage.at(at(offset))={TileKind::Lava,0,0};
        place_prop(game.stage,at({-2,-4}),PropKind::ScrapBin);
        place_prop(game.stage,at({-2,1}),PropKind::OreBin);
        return crane;
    }
    return {};
}
