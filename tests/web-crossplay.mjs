...
async function page(query){const p=await browser.newPage({viewport:{width:960,height:720}});p.on('pageerror',e=>console.log('BROWSER ERROR',e.message));p.on('console',m=>{if(m.text().includes('[Teeming]'))console.log(m.text().slice(0,300));});await p.goto(base+'/?'+query);await p.click('#play');await p.waitForFunction(()=>window.teeming?.gameState,{timeout:45000});return p;}
