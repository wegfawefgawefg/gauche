...
const processes=[];
function native(args,label){const child=spawn(root+'/build-release/teeming',args,{cwd:root,env:{...process.env,SDL_VIDEODRIVER:'dummy',SDL_AUDIODRIVER:'dummy',XDG_DATA_HOME:'/tmp/teeming-crossplay-'+label,XDG_CONFIG_HOME:'/tmp/teeming-config-'+label}});processes.push(child);child.stdout.on('data',d=>console.log(label,String(d)));child.stderr.on('data',d=>console.log(label,'ERR',String(d)));return child;}
