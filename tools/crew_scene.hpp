...
inline void arrange_crew_scene(Game& game,Cosmetics& cosmetics,bool cutting,bool alarm) {
    ...
    player_state(game,0).online=true; game.run.spawn={17,16};
    ...
    Entity& player=*get_entity(game,player_state(game,0).controlled); player.owner=0; player.facing={0,-1}; player.cell={18,13};
    ...
    for (Entity& actor:game.entities) {
        ...
        if (alarm) { actor.entity_b=player_state(game,0).controlled; actor.attack_wait=600; }
    }
