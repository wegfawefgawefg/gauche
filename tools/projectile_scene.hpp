...
inline void arrange_projectile_scene(Game& game, Cosmetics& cosmetics) {
    ...
    Entity& player = *get_entity(game, player_state(game,0).controlled); player.owner = 0; player.facing = {1, 0};
    launch_projectile(game, player_state(game,0).controlled.slot, make_item(ItemKind::Bow), {1, 0}, 14);
    launch_projectile(game, player_state(game,0).controlled.slot, make_item(ItemKind::Bomb), {0, 1}, 3);
...
inline void arrange_ballistics_scene(Game& game, Cosmetics& cosmetics) {
    ...
    const int owner = player_state(game,0).controlled.slot;
