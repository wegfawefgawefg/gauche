#include "belt_scene.hpp"
#include "rivet_scene.hpp"
...
#include "bridge_scene.hpp"
#include "fork_scene.hpp"
#include "stillwater_scene.hpp"
#include "mask_scene.hpp"
#include "anchor_scene.hpp"
#include "shelter_scene.hpp"
#include "sled_scene.hpp"
...
int main(int argc, char** argv) {
    ...
    if (mode!="quarry" && mode!="quarry-items" && mode!="hammer-items" && mode!="grates" && mode!="rivets" && mode!="rivet-items" && mode!="belts" && mode!="belt-items") {
        player.inventory = {};
    ...
    if (mode.starts_with("work-whistle")) {
        player.inventory={};
        insert_item(player.inventory,make_item(ItemKind::ForemanWhistle));
        insert_item(player.inventory,make_item(ItemKind::ForemanWhistle,1,ItemAttribute::Long));
        insert_item(player.inventory,make_item(ItemKind::ForemanWhistle,1,ItemAttribute::Durable));
    }
    ...
    if (mode.starts_with("sled")) arrange_sled_scene(game,cosmetics,player);
    if (mode.starts_with("shelter")) arrange_shelter_scene(game,cosmetics,player);
    if (mode.starts_with("anchor")) arrange_anchor_scene(game,cosmetics,player,mode=="anchor-blocked");
    if (mode.starts_with("mask")) arrange_mask_scene(game,cosmetics,player,mode!="mask-off");
    if (mode.starts_with("stillwater")) arrange_stillwater_scene(game,cosmetics,player,mode!="stillwater-flow");
    if (mode.starts_with("fork")) arrange_fork_scene(game,cosmetics,player,mode=="fork-pulse");
    if (mode.starts_with("bridge")) arrange_bridge_scene(game,cosmetics,player,mode=="bridge-broken",mode=="bridge-fire");
