...
// Static display of tunneling stages and swap-particle colors; no simulation ticks run.
inline void arrange_root_relics(Game& game, Cosmetics& cosmetics, Entity& player) {
    ...
    player.inventory = {};
    for (ItemKind kind : {ItemKind::Fist, ItemKind::SwapSeed, ItemKind::RootDrill, ItemKind::Pickaxe})
    ...
    Entity* swap = get_entity(game, spawn_entity(game, EntityKind::Projectile, {15, 11}));
    swap->label_a = static_cast<int>(ProjectileKind::Swap);
    swap->sprite = Sprite::SwapSeed;
    swap->facing = {1, 0};
    swap->counter_a = 3; swap->timer_b = 2; swap->timer_a = 20;
    swap->ground_item = make_item(ItemKind::SwapSeed);
    swap->light = {2, 250, {211, 164, 242}};
}
